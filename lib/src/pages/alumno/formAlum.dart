...
class FormAlumno extends StatefulWidget {
  ...
  FormAlumno({super.key, this.idAlumno});
...
class _FormAlumnoState extends State<FormAlumno> {
  ...
  Future _listarGrupos() async {
    try {
      final response =
          await http.post(Uri.parse("http://192.168.1.81/justy/leergrupo.php"));
  ...
  Future<int> _editarAlumno(String numeroControlOriginal, numeroControlCambio,
      String nombre, String grupo, String semestre, String turno,  String especialidad) async {
    try {
      final response = await http
          .post(Uri.parse("http://192.168.1.81/justy/editaralumno.php"), body: {
  ...
  Future<dynamic> _buscarAlumno(String idAlumno) async {
    try {
      final response = await http.post(
          Uri.parse("http://192.168.1.81/justy/buscaralumno.php"),
            ...
            child: Form(
              child: Column(
                children: <Widget>[
                  ...
                  _crearNumcontrol(
                      snapshot != null ? snapshot.data[0]["numCon"] : null),
                  SizedBox(
                    height: 20.0,
                  ),
                  _crearNombre(
                  ...
                  _grupo(
                      snapshot != null ? snapshot.data[0]["nomenclatura"] : null),
                  const SizedBox(
                    height: 20.0,
                  ),
                  _semestre(
                  ...
                  SizedBox(
                   ...
                   _turno(
                      snapshot != null ? snapshot.data[0]["turno"] : null),
                  SizedBox(
                    height: 20.0,
                  ),
                  _especialidad(snapshot != null
                      ? snapshot.data[0]["especi"]
                      : null),
                  const SizedBox(
                    height: 30.0,
                  ),
  ...
  List<DropdownMenuItem<String>> getOpcionesSemestre() {
  ...
  List<DropdownMenuItem<String>> getOpcionesGrupo(List<dynamic> _grupos) {
    ...
    _grupos.forEach((opcion1) {
      print(opcion1);
      print(opcion1["nomenclatura"]);
      lista2.add(DropdownMenuItem(
        child: Text(opcion1["nomenclatura"]),
        value: opcion1["nomenclatura"],
      ));
    });
 ...
 List<DropdownMenuItem<String>> getOpcionesturno() {
  ...
  Widget _turno(String? especialidad) {
      ...
      onPressed: () async {
        ...
        if (widget.idAlumno != null) {
          if (await _editarAlumno(widget.idAlumno!, _numeroControl.text,
                  _nombre.text, _semestreSele, _groupSele, _espeSele, _turnSele) >
              0) {
